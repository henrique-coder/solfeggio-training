...
const translations = {
  // General UI
  title: {
    Brazil: "Treinamento de Solfejo",
    "United States": "Solfeggio Training",
  },
  scoreLabel: { Brazil: "Pontuação:", "United States": "Score:" },
  loadingFonts: {
    Brazil: "Carregando fontes...",
  ...
  configButtonClose: {
    Brazil: "Fechar Config.",
    "United States": "Close Settings",
  },
  ...
  configSubPanelHotkeys: {
    Brazil: "Teclas de Atalho",
    "United States": "Hotkeys",
  },
  configPrompt: {
    Brazil: 'Pressione a tecla para "{note}"... (ESC para cancelar)',
    "United States": 'Press the key for "{note}"... (ESC to cancel)',
  },
  changeButton: { Brazil: "Alterar", "United States": "Change" },
  cancelButton: { Brazil: "Cancelar", "United States": "Cancel" },
  clearButtonTooltip: {
    Brazil: "Limpar Tecla",
    "United States": "Clear Hotkey",
  },

  // Hotkey Alerts
  alertKeyInUse: {
    Brazil:
      'A tecla "{key}" já está atribuída à nota "{existingNote}". Removendo atribuição anterior.',
    "United States":
      'The key "{key}" is already assigned to the note "{existingNote}". Removing previous assignment.',
  },
  alertInvalidKey: {
    Brazil:
      "Tecla inválida. Use letras, números, F1-F12, setas ou símbolos comuns.",
    "United States":
      "Invalid key. Use letters, numbers, F1-F12, arrow keys, or common symbols.",
  },

  // Range Adjuster
  adjustRange: {
    Brazil: "Ajustar Extensão das Notas:",
    "United States": "Adjust Note Range:",
  },
...
const getInitialHotkeyMappings = (notes) => {
  try {
    ...
    if (savedMappings) {
      ...
      if (Array.isArray(notes)) {
        ...
        console.warn(
          "getInitialHotkeyMappings received non-array 'notes'. Using empty mappings.",
        );
        return {};
      }
  ...
  if (Array.isArray(notes)) {
    ...
    console.warn(
      "getInitialHotkeyMappings received non-array 'notes' for default generation. Using empty mappings.",
    );
  }
...
function App() {
  ...
  const [hotkeyMappings, setHotkeyMappings] = useState(() =>
    getInitialHotkeyMappings(currentNotes),
  );
  ...
  const t = useCallback(
    (key, replacements = {}) => {
      ...
      let text =
        ...
        translations[key]?.["United States"] ||
        key; // Fallback
  ...
  const drawNote = useCallback(
    (noteName) => {
      ...
      const elementsToRemove = context.svg.querySelectorAll(
        ".vf-stavenote, .vf-voice, .vf-accidental, .vf-annotation, .vf-beam, .vf-modifiercontext",
      );
      ...
      try {
        ...
        if (noteName.includes("#") || noteName.includes("b")) {
          note.addAccidental(
            0,
            new VF.Accidental(noteName.includes("#") ? "#" : "b"),
          );
        }
        ...
        console.error("Error drawing main note:", noteName, error);
      }
    },
    [fontsReady, clef],
  );
  ...
  useEffect(() => {
    if (fontsReady && staffRef.current) {
      ...
      try {
        ...
        staveRef.current = new VF.Stave(10, 10, 580)
          .addClef(clef)
        ...
        if (currentNote) {
          setTimeout(() => drawNote(currentNote), 0);
        }
      } catch (error) {
        console.error("Error initializing main stave:", error);
      }
    }
  ...
  const drawRangeNotes = useCallback(() => {
    ...
    if (
      !fontsReady ||
      !rangeContextRef.current ||
      !rangeStaveRef.current ||
      !lowNote ||
      !highNote ||
      !clef
    ) {
    ...
    for (let i = lowIndex; i <= highIndex; i++) {
      visibleNotes.push(
        `${PITCHES[i % PITCHES.length]}/${Math.floor(i / PITCHES.length)}`,
      );
    }
    ...
    if (visibleNotes.length > maxNotesToShow) {
      console.warn(
        `Range too large (${visibleNotes.length} notes), displaying boundaries only.`,
      );
    ...
    try {
      ...
      const voice = new VF.Voice({
        num_beats: Math.max(1, Math.ceil(staveNotes.length / 4)),
        beat_value: 4,
      });
  ...
  useEffect(() => {
    ...
    console.log(
      "Range stave effect: Running. Current Clef:",
      clef,
      "Last Clef:",
      lastRangeClefRef.current,
    );

    let needsInitialization =
      !rangeRendererRef.current || clef !== lastRangeClefRef.current;
    ...
    if (needsInitialization) {
      ...
      try {
        rangeRendererRef.current = new VF.Renderer(
          rangeStaffRef.current,
          VF.Renderer.Backends.SVG,
        );
        ...
        rangeStaveRef.current
          .addClef(clef)
          .setContext(rangeContextRef.current)
          .draw(); // Draw clef
    ...
    if (rangeContextRef.current && rangeStaveRef.current) {
      ...
      console.warn(
        "Range stave effect: Context or Stave ref is missing, cannot draw notes yet.",
      );
    }
  ...
  const checkAnswer = useCallback(
    (selectedNoteName) => {
      if (!currentNote || configuringNote) return;
      ...
      const button = Array.from(
        ...
        (btn) =>
          btn.textContent.trim().split(" ")[0].toLowerCase() ===
          selectedNoteName.toLowerCase(),
      );
  ...
  ); // Added configuringNote dependency

  // --- Hotkey Handling ---
  const activeHotkeysString = useMemo(() => {
    return Object.values(hotkeyMappings)
      .filter((key) => key !== null)
      .join(",");
  }, [hotkeyMappings]);
  ...
  useHotkeys(
    ...
    (event, handler) => {
      ...
      const noteForPressedKey = Object.entries(hotkeyMappings).find(
        ([_, mappedKey]) => mappedKey === pressedKey,
      );
  ...
  const moveNote = useCallback(
    (isLow, direction) => {
      ...
      if (direction === "up") {
        newPitchIndex++;
        if (newPitchIndex >= PITCHES.length) {
          newPitchIndex = 0;
          newOctaveNum++;
        }
      } else {
        newPitchIndex--;
        if (newPitchIndex < 0) {
          newPitchIndex = PITCHES.length - 1;
          newOctaveNum--;
        }
      }
      if (
        newOctaveNum < MIN_OCTAVE ||
        (newOctaveNum === MIN_OCTAVE && newPitchIndex < PITCHES.indexOf("C"))
      )
        return;
      if (
        newOctaveNum > MAX_OCTAVE ||
        (newOctaveNum === MAX_OCTAVE && newPitchIndex > PITCHES.indexOf("B"))
      )
        return;
    ...
    [lowNote, highNote, configuringNote],
  ); // Added configuringNote dependency
    ...
    <div className="App">
      ...
      <div className="range-adjuster">
        ...
        <div className="range-buttons">
          ...
          <button
            onClick={() => moveNote(false, "down")}
            title={t("highNoteDown")}
            disabled={!!configuringNote}
          >
            {" "}
            {t("highNoteDown")}{" "}
      ...
      <div className="footer-link">
        <a
          href="https://github.com/henrique-coder/solfeggio-training"
          target="_blank"
